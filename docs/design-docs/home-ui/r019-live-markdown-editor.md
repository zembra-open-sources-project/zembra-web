...
- tag suggestion 在输入至少一个 tag 字符后出现，并支持已有 tag 选择和独立 tag 创建。
...
| In Scope | 无匹配时显示创建项，并调用独立 tag 创建接口。 |
| In Scope | 新增 taxonomy 创建 tag client，接口路径、请求体和 workspace scope 在实现前通过实时 OpenAPI 确认。 |
| In Scope | 覆盖创建态、编辑态、tag suggestion、创建 tag、Markdown 字符串输出和提交 payload 的测试。 |
...
| Milkdown | 备选 | Markdown WYSIWYG 方向贴合需求，但需要验证当前项目中 tag suggestion、独立创建 tag 和纯字符串状态的集成成本。 |
...
| `src/api/taxonomy.client.ts` | 在 `TaxonomyClient` 中新增创建 tag 方法，具体路径和请求体以实时 OpenAPI 为准。 |
| `src/features/notes/noteStore.ts` | 暴露创建 tag action 或复用 taxonomy client 更新 tag 列表，保证新建 tag 后 composer 和 sidebar 使用同一份 tags。 |
| `src/pages/home/HomePage.tsx` | 将 `tags`、创建 tag 回调和必要编辑器配置传给 `NoteEditor`，创建和编辑提交逻辑继续基于字符串解析。 |
...
flowchart TD
  ...
  I --> J["taxonomyClient.createTag via store action"]
  J --> K["tags list updated"]
  H --> L["Replace token with #path chip"]
  K --> L
  A --> M["Submit note"]
  M --> N["parseFieldNames / parseTagNames / parseNoteLinks"]
  N --> O["createNote / updateNote"]
```
...
| Markdown 能力 | 编辑态要求 |
| --- | --- |
| 标题和段落 | 输入和粘贴 Markdown 后实时显示为对应 block。 |
...
| 引用和分割线 | 以 block 结构实时显示。 |
| 链接 | 显示为链接 mark，输出保持 Markdown link。 |
| 行内代码和代码块 | 实时显示对应 code 样式，不新增语法高亮库。 |
...
| 创建 tag | 调用独立 tag 创建接口，成功后插入新 tag chip，并更新本地 tag 列表。 |
| 创建失败 | 保持当前输入 token 不丢失，显示轻量错误状态或沿用现有错误反馈策略。 |

### 独立 Tag 创建接口

当前仓库只实现了 `GET /tags?all=true` 和 note tag 查询，未发现前端已有独立创建 tag client。实现前必须读取实时 OpenAPI 或后端文档，确认创建 tag 的路径、请求体、响应体、workspace scope 和层级 path 规则。设计上 `TaxonomyClient` 增加创建 tag 方法，返回 `TagDto`，由 store 合并进 `tags` 列表。

接口形态不得凭经验捏造。若 OpenAPI 没有独立 tag 创建接口，必须停止实现并反馈后端契约缺失，因为用户已确认“创建 #xxx”必须调用独立 tag 创建接口。
...
| `home` | `composer.tagSuggestion.error` | 无 | 创建标签失败 | 建立標籤失敗 | Failed to create tag |
...
| API 测试 | `taxonomyClient.createTag()` 按实时 OpenAPI 发送正确路径、workspace scope 和请求体，并映射返回的 `TagDto`。 |
| Store 测试 | 创建 tag 成功后将新 tag 合并进 `tags` 列表，不丢失已有 tags。 |
...
| 组件测试 | 无匹配时显示“创建 #xxx”，选择后调用创建 tag action，成功后插入 tag chip。 |
...
| 手工检查 | 在创建态和编辑态输入列表、任务列表、引用、表格、代码、链接和 tag，确认实时渲染、suggestion、创建 tag 和提交后展示一致。 |
