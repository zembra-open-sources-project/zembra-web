...
用户验收发现当前编辑栏按钮没有实时预览。该问题属于 r019 的补充实现范围，因为 r019 已要求编辑态实时渲染 Markdown，并要求现有 tag、field、bold、list 工具按钮继续作用于新编辑器。补充验收标准为：点击加粗按钮时编辑器应进入粗体输入状态或将选区切换为粗体 mark，不应只插入裸 `****`；点击列表按钮时编辑器应进入列表结构，不应只插入普通 `- ` 文本；点击标签或 Field 按钮时仍插入对应前缀并保持字符串同步。该补充不改变输入框布局、不新增工具按钮、不改变 note content 的 Markdown 存储契约。
